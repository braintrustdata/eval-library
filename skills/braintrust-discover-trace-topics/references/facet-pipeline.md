...
it graduates into a classifier or a scorer with its own validation — see `braintrust-validate-eval-scorer`.
...
The ground-truth run uses a strong model where `braintrust-validate-eval-scorer` would use adjudicated human
