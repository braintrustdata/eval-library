...
still produces a per-group score you can localize. When `braintrust-write-eval-scorer` says per-item scoring
...
`braintrust-discover-trace-topics`.

## LLM evaluator defaults

- **Emit classes, map to numbers.** Have the model return semantic labels and configure
  `choice_scores` to map them. Full rationale in `braintrust-write-eval-scorer/references/scorer-patterns.md`.
...
where rare-but-severe slices matter — `braintrust-monitor-production-evals` owns that argument. `[platform]`
