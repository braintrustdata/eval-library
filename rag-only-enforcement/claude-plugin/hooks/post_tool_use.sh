...
if [[ "$TOOL_NAME" == "Bash" ]] || [[ "$TOOL_NAME" == "Terminal" ]]; then
    ...
    if [[ "$CMD" == *"vector_search.py"* ]]; then
        mark_vector_search_used "$SESSION_ID"
        debug "Vector search detected and marked"
    fi
fi

# Tool gating: Block ANY terminal tool usage until vector search has been used
if is_agentic_search_tool "$TOOL_NAME"; then
    if should_gate_tools "$SESSION_ID"; then
        if ! has_vector_search_been_used "$SESSION_ID"; then
            CMD=$(echo "$TOOL_INPUT" | jq -r '.command // empty' 2>/dev/null | head -c 100)
            log "ERROR" "Tool gating: BLOCKED $TOOL_NAME command - vector search must be used first before any terminal operations"
            log "ERROR" "Command was: $CMD"
            debug "Tool gating: Terminal command blocked until vector search is used"
            # Print error to stdout so Claude Code sees it
            echo "ERROR: Tool blocked by policy - You must use vector search first before running terminal commands. Use vector_search.py to find relevant files."
            exit 1
        fi
    fi
fi
